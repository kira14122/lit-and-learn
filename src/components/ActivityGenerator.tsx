...
const INSTR = {
  ...
  matching:  'Match each word from the text with its correct definition. Write the letter next to the number.',
...
// existing QuickVocabActivity / QuickGrammarActivity shapes IN CODE, so the
// exercise formatting can never be malformed by the model.

type VocabExType   = 'gap-fill' | 'multiple-choice' | 'own-sentence';
type GrammarExType = 'gap-fill' | 'multiple-choice' | 'error-correction';

const VOCAB_EX_OPTIONS: { id: VocabExType; label: string }[] = [
  { id: 'gap-fill',        label: 'Gap-Fill (from Word Bank)' },
  { id: 'multiple-choice', label: 'Multiple Choice' },
  { id: 'own-sentence',    label: 'Write Your Own Sentence' },
];
const GRAMMAR_EX_OPTIONS: { id: GrammarExType; label: string }[] = [
  { id: 'gap-fill',         label: 'Contextual Gap-Fill' },
  { id: 'multiple-choice',  label: 'Multiple Choice' },
  { id: 'error-correction', label: 'Error Correction' },
];

/** Split `total` items across `parts` blocks as evenly as possible (no waste). */
const distribute = (total: number, parts: number): number[] => {
  if (parts <= 0) return [];
...
function assembleQuickVocab(raw: RawVocabData, types: VocabExType[]): QuickVocabActivity {
  const glossary = raw.items.map(it => ({
    word: it.target,
    pos: it.pos,
    definition: it.definition,
    example: it.example, // dedicated model sentence — never the exercise sentence (may be '')
  }));

  const sizes = distribute(raw.items.length, types.length);
  const exercises: GrammarExercise[] = [];
  let cursor = 0;

  types.forEach((t, i) => {
    const slice = raw.items.slice(cursor, cursor + sizes[i]);
    cursor += sizes[i];
    ...
    if (t === 'gap-fill') {
      exercises.push({
        ...
        instruction: 'Complete each sentence with the correct word or phrase from the Word Bank above.',
        items: slice.map(it => ({ q: it.contextSentence.replace('[TARGET]', '___'), answer: it.target })),
      });
    } else if (t === 'multiple-choice') {
      exercises.push({
        title: 'Choose the Correct Option',
        instruction: 'Circle the option that best completes each sentence.',
        items: slice.map(it => ({
          q: `${it.contextSentence.replace('[TARGET]', '___')} (${shuffle([it.target, ...it.distractors]).join(' / ')})`,
          answer: it.target,
        })),
      });
    } else {
      exercises.push({
        ...
        instruction: 'Write an original sentence using each word or phrase. Make the meaning clear.',
        items: slice.map(it => ({ q: `${it.target} — ${it.definition}`, answer: 'Answers will vary.' })),
      });
    }
  });

  return { theme: raw.theme, glossary, exercises };
}

function assembleQuickGrammar(raw: RawGrammarData, types: GrammarExType[]): QuickGrammarActivity {
  // Reserve items as model examples only when we can afford it: 2 when plentiful,
  // 1 when moderate, 0 when scarce — the drills always come first. Reserved
  // examples never appear in exercises, so they can't give answers away.
  const reserve = raw.items.length >= 10 ? 2 : raw.items.length >= 8 ? 1 : 0;
  const exampleItems = raw.items.slice(0, reserve);
  const pool = raw.items.slice(reserve);
  const examples = exampleItems.map(it => it.contextSentence.replace('[TARGET]', it.targetPhrase));

  const sizes = distribute(pool.length, types.length);
  const exercises: GrammarExercise[] = [];
  let cursor = 0;

  types.forEach((t, i) => {
    const slice = pool.slice(cursor, cursor + sizes[i]);
    cursor += sizes[i];
    if (slice.length === 0) return;
    ...
    if (t === 'gap-fill') {
      exercises.push({
        ...
        instruction: `Complete each sentence using the correct form of ${raw.grammarPoint}.`,
        items: slice.map(it => {
          // Negative answers need a signal in the hint — "(not / be)" is the
          // standard ESL convention; "(be)" alone makes "hadn't been" unguessable.
          const isNegative = /n['’]t\b|\bnot\b/i.test(it.targetPhrase);
          const hint = it.hint ? `(${isNegative ? 'not / ' : ''}${it.hint})` : '';
          return {
            q: it.contextSentence.replace('[TARGET]', hint ? `___ ${hint}` : '___'),
            answer: it.targetPhrase,
          };
        }),
      });
    } else if (t === 'multiple-choice') {
      exercises.push({
        ...
        instruction: 'Circle the option that best completes each sentence.',
        items: slice.map(it => ({
          q: `${it.contextSentence.replace('[TARGET]', '___')} (${shuffle([it.targetPhrase, ...it.distractors]).join(' / ')})`,
          answer: it.targetPhrase,
        })),
      });
    } else {
      exercises.push({
        ...
        instruction: 'Each sentence contains one grammar mistake. Rewrite the sentence correctly on the line.',
        items: slice.map(it => ({
          q: it.incorrectSentence,
          answer: it.contextSentence.replace('[TARGET]', it.targetPhrase),
        })),
      });
    }
  });

  return { grammarPoint: raw.grammarPoint, rule: raw.ruleText, examples, exercises };
}
...
export const ActivityGenerator = () => {
  ...
  const [vocabExTypes, setVocabExTypes]     = useState<VocabExType[]>(['gap-fill', 'multiple-choice']);
  const [grammarExTypes, setGrammarExTypes] = useState<GrammarExType[]>(['gap-fill', 'error-correction']);
  ...
  const genQuickVocab = async () => {
    ...
    let qv: QuickVocabActivity | null = raw ? assembleQuickVocab(raw, vocabExTypes) : null;
    if (!qv) qv = await generateQuickVocab(quickTheme, level, teacherFocus); // legacy fallback
    if (qv) {
      setSections([{ id: uid(), kind: 'quickVocab', pageBreakBefore: false, qv }]);
  ...
  const genQuickGrammar = async () => {
    ...
    let data: QuickGrammarActivity | null = raw ? assembleQuickGrammar(raw, grammarExTypes) : null;
    if (!data) data = await generateQuickGrammar(grammarPoint, level, teacherFocus); // legacy fallback
    if (data) {
      setSections([{ id: uid(), kind: 'quickGrammar', pageBreakBefore: false, data }]);
      // Quick generation replaces the whole sheet: refresh the title unless the teacher typed a custom one.
  ...
  // Numbers must follow RENDER order: TF/Comp → Vocab → Crossword → Discussion → Grammar
              ...
              <div style={{ borderTop:'1px solid #E2E8F0', paddingTop:'14px' }}>
                ...
                {quickType === 'vocab' && (
                  <>
                    ...
                    <label style={lbl}>Exercise Types</label>
                    <div style={{ marginBottom:'10px' }}>
                      {VOCAB_EX_OPTIONS.map(opt => (
                        <label key={opt.id} style={{ display:'flex', alignItems:'center', gap:'9px', cursor:'pointer', fontSize:'0.8rem', color:'#475569', fontWeight:500, marginBottom:'6px' }}>
                          <input type="checkbox" checked={vocabExTypes.includes(opt.id)}
                            onChange={() => setVocabExTypes(cur => cur.includes(opt.id) ? cur.filter(t => t !== opt.id) : [...cur, opt.id])}
                            style={{ width:'14px', height:'14px', accentColor:'#EA580C', cursor:'pointer', flexShrink:0 }} />
                          {opt.label}
                        </label>
                      ))}
                    </div>
                ...
                {quickType === 'grammar' && (
                  <>
                    ...
                    <label style={lbl}>Exercise Types</label>
                    <div style={{ marginBottom:'10px' }}>
                      {GRAMMAR_EX_OPTIONS.map(opt => (
                        <label key={opt.id} style={{ display:'flex', alignItems:'center', gap:'9px', cursor:'pointer', fontSize:'0.8rem', color:'#475569', fontWeight:500, marginBottom:'6px' }}>
                          <input type="checkbox" checked={grammarExTypes.includes(opt.id)}
                            onChange={() => setGrammarExTypes(cur => cur.includes(opt.id) ? cur.filter(t => t !== opt.id) : [...cur, opt.id])}
                            style={{ width:'14px', height:'14px', accentColor:'#DC2626', cursor:'pointer', flexShrink:0 }} />
                          {opt.label}
                        </label>
                      ))}
                    </div>

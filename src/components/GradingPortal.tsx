...
const IconSave     = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><path d="M17 21v-8H7v8M7 3v5h8"></path></svg>);
...
export const GradingPortal: React.FC<{
  ...
  // Re-runs the draft-key reads after a draft is written, loaded or discarded.
  ...
  // Term summary folds away like the other two analysis panels. During a sitting
  // it is scrolled past on every student; one click when it is wanted.
  const [showTermSummary, setShowTermSummary] = useState(false);


  // Term picker. Cosmetic until student_grades gains a `term` column — the
  // choice is remembered but nothing filters on it yet (see add-term.sql).
  // Three terms a year: Fall runs Sep–Dec, Winter Jan–Apr, Summer May–Aug.
  const TERM_OF_MONTH = (m:number) => m>=8 ? 'Fall' : m>=4 ? 'Summer' : 'Winter';
  const currentTerm = () => {
    const d = new Date();
    return `${TERM_OF_MONTH(d.getMonth())} ${d.getFullYear()}`;
  };
  const TERM_OPTIONS = (() => {
    const y = new Date().getFullYear(); const out: string[] = [];
    for (const yr of [y-1, y, y+1]) ['Winter','Summer','Fall'].forEach(t=>out.push(`${t} ${yr}`));
    return out;
  })();
  ...
  useEffect(() => {
    if (!teacherEmail || !currentLevel) { setLessonMap({}); return; }
    let cancelled = false;
    (async () => {
      try {
        const supabase = getSupabaseClient((await getToken({template:'supabase'}))||'');
        const { data } = await supabase.from('test_lessons').select('skill,lessons')
          .eq('teacher_email', teacherEmail).eq('term', termLabel)
  ...
  const scoreBoxes = useRef<HTMLDivElement|null>(null);
  const focusFirstScore = () => {
    const el = scoreBoxes.current?.querySelector<HTMLInputElement>('[data-score-box]');
    el?.focus(); el?.select();
  };
  ...
  const scoreKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key !== 'Enter') return;
    e.preventDefault();
    const boxes = Array.from(scoreBoxes.current?.querySelectorAll<HTMLInputElement>('[data-score-box]') || []);
    const at = boxes.indexOf(e.currentTarget);
    if (at > -1 && at < boxes.length - 1) { boxes[at+1].focus(); boxes[at+1].select(); }
    else document.querySelector<HTMLTextAreaElement>('[data-feedback-box]')?.focus();
  };
  ...
  // Changing term reloads the records and drops the current student, so nothing
  // from the previous term is left on screen against the new one.
  const firstTermRun = useRef(true);
  useEffect(() => {
    if (firstTermRun.current) { firstTermRun.current = false; return; }
    fetchAllGrades();
    setSelectedStudent(null);
    showToast(`Now showing ${termLabel}`, 'success');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [termLabel]);
  useEffect(() => {
    ...
    if (selectedStudent) setTimeout(focusFirstScore, 0);
  ...
  const fetchAllGrades = async () => {
    const supabase = getSupabaseClient((await getToken({template:'supabase'}))||'');
    const {data} = await supabase.from('student_grades').select('*').eq('term',termLabel).order('created_at',{ascending:false});
    if (data) setAllGrades(data);
  };
  const fetchStudentHistory = async (id:string) => {
    const supabase = getSupabaseClient((await getToken({template:'supabase'}))||'');
    const {data} = await supabase.from('student_grades').select('*').eq('user_id',id).eq('term',termLabel).order('created_at',{ascending:false});
    if (data) setStudentHistory(data);
  };

  // ── Draft management (localStorage only) ──────────────────────────────────
  const makeDraftKey = (sid:string, aName:string) => `ll_draft_${termLabel}_${sid}_${aName}`;
  ...
  // The plan for the student on screen: which skill is weakest, which lessons
  // are suggested, and the email that would be written.
  const feedbackPlan = useMemo(() => {
    if (!selectedStudent) return null;
    ...
    return buildFeedback({
      ...
      lessonsBySkill: lessonMap,
      // The student's most recent earlier test this term, for the improvement
      // line. studentHistory is already term-scoped and newest first.
      previous: (() => {
        const order = ['First Test','Midterm','Third Test','Final Test'];
        const here = order.indexOf(assessmentName);
        const earlier = (studentHistory||[])
          .filter((h:any)=>{ const i = order.indexOf(h.assessment_name); return i > -1 && (here === -1 || i < here); })
          .sort((a:any,b:any)=>order.indexOf(b.assessment_name)-order.indexOf(a.assessment_name))[0];
        if (!earlier) return null;
        let p:any={}; try{ p = JSON.parse(earlier.score)||{}; }catch{ return null; }
        if (p.isAbsent || p.notApplicable || !p.maxPoints) return null;
        return {
  ...
  const PORTAL_CSS = `
    .gp-root { font-family: 'Inter', system-ui, sans-serif; }
    .gp-root h3, .gp-root h4 { font-family: 'Fredoka', system-ui, sans-serif !important; font-weight: 500 !important; }
    .gp-root button { font-family: 'Fredoka', system-ui, sans-serif !important; border-radius: 999px !important; }
    .gp-root button[style*="border-radius: 50%"] { border-radius: 50% !important; }
    .gp-root input, .gp-root textarea { font-family: 'Inter', system-ui, sans-serif !important; border-radius: 12px !important; }

    /* Native selects render an OS menu that ignores the page's design — dark grey
       on macOS, a different grey on Windows. Styling the control itself is the
       only part we own, so make it unmistakably a control: our own chevron, our
       own border, no platform arrow. */
    .gp-root select {
      font-family: 'Fredoka', system-ui, sans-serif !important;
      border-radius: 12px !important;
      -webkit-appearance: none; -moz-appearance: none; appearance: none;
      background-image: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%235A6180' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpath d='M6 9l6 6 6-6'/%3e%3c/svg%3e") !important;
    ...
    .gp-root select:hover { border-color: #C9C6F7 !important; }
    .gp-root select:focus { border-color: #4F46E5 !important; box-shadow: 0 0 0 3px #EEEDFE; outline: none; }
    .gp-root select:disabled { background-color: #F7F8FD !important; color: #6C7391 !important; cursor: not-allowed; }
    .gp-root input:focus, .gp-root textarea:focus { border-color: #4F46E5 !important; box-shadow: 0 0 0 3px #EEEDFE; outline: none; }

    /* Section rules: strong enough to read as a division, not as a hairline. */
    .gp-sep { border-top: 1px solid #DDE2EE; }
    .gp-root .gp-row { display:flex; align-items:flex-start; gap:10px; padding:10px 12px; border-radius:12px; cursor:pointer; border:1.5px solid transparent; }
    .gp-root .gp-row:hover { background:#F7F8FD; }
    .gp-root .gp-row.on { background:#EEEDFE; border-color:#4F46E5; }
    .gp-root .gp-pip { width:8px; height:8px; border-radius:50%; flex-shrink:0; margin-top:6px; }
  `;
  ...
  <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(15,23,42,0.5)',backdropFilter:'blur(4px)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
    <div style={{background:'#fff',borderRadius:'24px',width:'100%',maxWidth:'640px',maxHeight:'85vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 40px rgba(0,0,0,0.15)'}}>
      ...
      {deleteTarget && (
        ...
        <div className="gp-root" style={{display:'grid',gridTemplateColumns:focused?'1fr':'minmax(300px, 380px) minmax(0, 1fr)',gap:'20px',alignItems:'start'}}>

          {/* ═══ Students ═══ */}
          {!focused && (
          <div style={{background:'#fff',borderRadius:'18px',boxShadow:'0 2px 8px rgba(27,31,59,0.05)',padding:'18px',position:'sticky',top:'20px'}}>

            <div style={{display:'flex',gap:'4px',background:'#EEF0F8',padding:'4px',borderRadius:'999px',marginBottom:'16px'}}>
              {([['directory','Active'],['archived','Archived'],['manual','Add']] as any[]).map(([k,lbl])=>(
                <button key={k} onClick={()=>setGradingRosterMode(k)} style={{flex:1,padding:'7px 0',border:'none',borderRadius:'999px',fontSize:'0.8rem',fontWeight:500,cursor:'pointer',whiteSpace:'nowrap',background:gradingRosterMode===k?'#fff':'transparent',color:gradingRosterMode===k?'#4F46E5':'#5A6180',boxShadow:gradingRosterMode===k?'0 2px 8px rgba(27,31,59,0.05)':'none'}}>{lbl}</button>
              ))}
            </div>
            ...
            {gradingRosterMode!=='manual' ? (
              <>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'10px',marginBottom:'12px'}}>
                  <h3 style={{margin:0,fontSize:'1.05rem',color:'#1B1F3B'}}>{gradingRosterMode==='archived'?'Archived':'Students'}</h3>
                ...
                <div style={{display:'flex',gap:'8px',marginBottom:'8px'}}>
                  <select value={gradingFilterLevel} onChange={e=>setGradingFilterLevel(e.target.value)} style={{flex:1,minWidth:0,padding:'9px 10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.82rem',color:'#1B1F3B',background:'#fff'}}>
                    <option value="All Levels">All levels</option>
                    {['Level 1','Level 2','Level 3','Level 4','Level 5','Level 6','Level 7','Level 8','Business English'].map(l=><option key={l}>{l}</option>)}
                  </select>
                  <select value={gradingFilterTime} onChange={e=>setGradingFilterTime(e.target.value)} style={{flex:1,minWidth:0,padding:'9px 10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.82rem',color:'#1B1F3B',background:'#fff'}}>
                    <option value="All Times">All times</option>
                    {['Morning Class','Evening Class','Weekend Class'].map(t=><option key={t}>{t}</option>)}
                  </select>
                </div>

                <input type="text" placeholder="Search" value={gradingSearchQuery} onChange={e=>setGradingSearchQuery(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'10px 12px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.88rem',color:'#1B1F3B',marginBottom:'10px'}}/>

                {gradingFilteredStudents.length===0
                  ? <div style={{padding:'28px 12px',textAlign:'center',color:'#6C7391',fontSize:'0.85rem'}}>{gradingRosterMode==='archived'?'No archived students.':'No students match.'}</div>
                  : <div style={{display:'flex',flexDirection:'column',gap:'2px',maxHeight:'52vh',overflowY:'auto',margin:'0 -6px',padding:'0 6px'}}>
                      {gradingFilteredStudents.map(student=>{
                        ...
                        return (
                          <div key={student.id} className={`gp-row${isSel?' on':''}`} onClick={isArchivedRow?undefined:()=>setSelectedStudent(student)}>
                            <span className="gp-pip" style={{background: cnt>0 ? '#0E9F6E' : hasDraftBadge ? '#B7791F' : '#E4E7F2'}}/>
                            <div style={{flexGrow:1,minWidth:0}}>
                              <div style={{fontWeight:500,fontSize:'0.9rem',color:isSel?'#4F46E5':'#1B1F3B',lineHeight:1.3,overflowWrap:'anywhere'}}>{student.full_name}</div>
                              <div style={{fontSize:'0.73rem',color:'#6C7391',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                                {[student.course_level,student.class_time].filter(Boolean).join(' \u00B7 ')||'\u2014'}
                                {hasDraftBadge && <span onClick={isArchivedRow?undefined:e=>{e.stopPropagation();setSelectedStudent(student);setAssessmentName(draftedAssessments[0]);}} style={{color:'#B7791F',cursor:'pointer'}}> \u00B7 draft</span>}
                              </div>
                            </div>
                            <div style={{display:'flex',alignItems:'center',gap:'2px',flexShrink:0,marginTop:'2px'}}>
                              {isArchivedRow
                                ? <button onClick={e=>{e.stopPropagation();handleRestoreStudent(student.id,student.full_name);}} title="Restore student" style={{width:'26px',height:'26px',display:'flex',alignItems:'center',justifyContent:'center',background:'transparent',border:'none',color:'#4F46E5',cursor:'pointer',padding:0}}>&#8630;</button>
                                : <button onClick={e=>{e.stopPropagation();handleArchiveStudent(student.id,student.full_name);}} title="Archive student" style={{width:'26px',height:'26px',display:'flex',alignItems:'center',justifyContent:'center',background:'transparent',border:'none',color:'#6C7391',cursor:'pointer',padding:0}}><IconArchive/></button>
                              }
                              <button onClick={e=>{e.stopPropagation();setDeleteTarget({type:'student',id:student.id,name:student.full_name});}} title="Remove student" style={{width:'26px',height:'26px',display:'flex',alignItems:'center',justifyContent:'center',background:'transparent',border:'none',color:'#E02424',cursor:'pointer',padding:0}}><IconTrash/></button>
                            </div>
                          </div>
                        );
                      })}
                ...
                <div style={{display:'flex',gap:'14px',fontSize:'0.75rem',color:'#5A6180',marginTop:'12px',paddingTop:'12px',borderTop:'1px solid #DDE2EE'}}>
                  <span><span className="gp-pip" style={{background:'#0E9F6E',display:'inline-block',marginRight:'5px'}}/>{gradingFilteredStudents.filter(s=>allGrades.some(g=>g.user_id===s.id)).length} graded</span>
                  <span><span className="gp-pip" style={{background:'#E4E7F2',display:'inline-block',marginRight:'5px'}}/>{gradingFilteredStudents.filter(s=>!allGrades.some(g=>g.user_id===s.id)).length} to do</span>
                </div>
              </>
            ) : (
              <div>
                <h3 style={{margin:'0 0 14px',color:'#1B1F3B',fontSize:'1.02rem'}}>Enroll new student</h3>
                <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                  <div><label style={{display:'block',fontSize:'0.8rem',fontWeight:500,marginBottom:'5px',color:'#5A6180'}}>Full name</label><input type="text" value={manualName} onChange={e=>setManualName(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'10px 12px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.9rem'}}/></div>
                  <div><label style={{display:'block',fontSize:'0.8rem',fontWeight:500,marginBottom:'5px',color:'#5A6180'}}>Email address</label><input type="email" value={manualEmail} onChange={e=>setManualEmail(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'10px 12px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.9rem'}}/></div>
                  <div style={{display:'flex',gap:'10px'}}>
                    <div style={{flex:1,minWidth:0}}><label style={{display:'block',fontSize:'0.8rem',fontWeight:500,marginBottom:'5px',color:'#5A6180'}}>Level</label><select value={manualLevel} onChange={e=>setManualLevel(e.target.value)} style={{width:'100%',padding:'10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.85rem',background:'#fff'}}>{['Level 1','Level 2','Level 3','Level 4','Level 5','Level 6','Level 7','Level 8','Business English'].map(l=><option key={l}>{l}</option>)}</select></div>
                    <div style={{flex:1,minWidth:0}}><label style={{display:'block',fontSize:'0.8rem',fontWeight:500,marginBottom:'5px',color:'#5A6180'}}>Class time</label><select value={manualTime} onChange={e=>setManualTime(e.target.value)} style={{width:'100%',padding:'10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.85rem',background:'#fff'}}>{['Morning Class','Evening Class','Weekend Class'].map(t=><option key={t}>{t}</option>)}</select></div>
                  </div>
                  <button onClick={handleAddStudentToRoster} style={{background:'#4F46E5',color:'#fff',padding:'12px',border:'none',fontWeight:500,cursor:'pointer',marginTop:'4px'}}>Add student</button>
                </div>
              </div>
            )}
          </div>
            ...
            {selectedStudent ? (
              <>
                <div style={{display:'flex',justifyContent:'flex-end',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
                  {focused && <>
                    <span style={{fontSize:'0.75rem',color:'#6C7391',marginRight:'2px'}}>{gradingIdx+1} / {gradingFilteredStudents.length}</span>
                    <button onClick={goPrevStudent} disabled={gradingIdx<=0} style={{width:'32px',height:'32px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#5A6180',cursor:gradingIdx<=0?'not-allowed':'pointer',opacity:gradingIdx<=0?0.4:1}}>&#8249;</button>
                    <button onClick={goNextStudent} disabled={gradingIdx>=gradingFilteredStudents.length-1} style={{width:'32px',height:'32px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#5A6180',cursor:'pointer',opacity:gradingIdx>=gradingFilteredStudents.length-1?0.4:1}}>&#8250;</button>
                  </>}
                  <button onClick={()=>setShowProgressReport(true)} style={{background:'#fff',color:'#4F46E5',border:'1.5px solid #DDE2EE',fontSize:'0.82rem',fontWeight:500,padding:'8px 14px',cursor:'pointer'}}>Progress report</button>
                  <button onClick={()=>setFocusMode(!focused)} style={{background:focused?'#4F46E5':'#fff',color:focused?'#fff':'#4F46E5',border:'1px solid '+(focused?'#4F46E5':'#E4E7F2'),fontSize:'0.82rem',fontWeight:500,padding:'8px 14px',cursor:'pointer'}}>{focused?'Exit focus':'Focus'}</button>
                </div>

                <div style={{background:'#fff',borderRadius:'18px',boxShadow:'0 2px 8px rgba(27,31,59,0.05)',overflow:'hidden'}}>

                  <div style={{padding:'20px 24px',borderBottom:'1px solid #DDE2EE',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'14px',flexWrap:'wrap'}}>
                    <div style={{minWidth:0}}>
                      <h3 style={{margin:0,fontSize:'1.5rem',color:'#1B1F3B',lineHeight:1.2}}>{selectedStudent.full_name}</h3>
                      <div style={{fontSize:'0.85rem',color:'#6C7391',marginTop:'2px'}}>{selectedStudent.email||'No email on file'}{selectedStudent.course_level?` \u00B7 ${selectedStudent.course_level}`:''}</div>
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
                      <span style={{fontSize:'0.72rem',fontWeight:500,padding:'4px 11px',borderRadius:'999px',background:hasBeenGraded?(existingEmailed?'#E6F7F0':'#FDF6E7'):'#EEF0F8',color:hasBeenGraded?(existingEmailed?'#0E9F6E':'#B7791F'):'#5A6180'}}>
                        {hasBeenGraded?(existingIsNA?'Not applicable':existingEmailed?'Sent':'Saved, not sent'):draftData?'Draft saved':'Not started'}
                      </span>
                      <button onClick={handleOpenProfileEditor} title="Edit profile" style={{background:'#fff',border:'1.5px solid #DDE2EE',padding:'6px 12px',color:'#5A6180',cursor:'pointer',fontSize:'0.8rem'}}>Edit</button>
                      <button onClick={()=>setSelectedStudent(null)} title="Close" style={{background:'#EEF0F8',border:'none',width:'30px',height:'30px',borderRadius:'50%',color:'#5A6180',cursor:'pointer'}}>&#10005;</button>
                    </div>
                  </div>

                  {isEditingProfile && (
                    <div style={{padding:'18px 24px',borderBottom:'1px solid #DDE2EE',background:'#F7F8FD'}}>
                      <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                        <input type="text" value={editProfileName} onChange={e=>setEditProfileName(e.target.value)} placeholder="Full name" style={{padding:'10px 12px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.9rem'}}/>
                        <input type="email" value={editProfileEmail} onChange={e=>setEditProfileEmail(e.target.value)} placeholder="Email" style={{padding:'10px 12px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.9rem'}}/>
                        <div style={{display:'flex',gap:'10px'}}>
                          <select value={editProfileLevel} onChange={e=>setEditProfileLevel(e.target.value)} style={{flex:1,padding:'10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.85rem',background:'#fff'}}>{['Level 1','Level 2','Level 3','Level 4','Level 5','Level 6','Level 7','Level 8','Business English'].map(l=><option key={l}>{l}</option>)}</select>
                          <select value={editProfileTime} onChange={e=>setEditProfileTime(e.target.value)} style={{flex:1,padding:'10px',border:'1.5px solid #DDE2EE',outline:'none',fontSize:'0.85rem',background:'#fff'}}>{['Morning Class','Evening Class','Weekend Class'].map(t=><option key={t}>{t}</option>)}</select>
                        </div>
                        <div style={{display:'flex',gap:'8px'}}>
                          <button onClick={()=>setIsEditingProfile(false)} style={{flex:1,padding:'10px',background:'#fff',border:'1.5px solid #DDE2EE',color:'#5A6180',fontWeight:500,cursor:'pointer'}}>Cancel</button>
                          <button onClick={handleSaveProfileUpdate} disabled={isSavingProfile} style={{flex:1,padding:'10px',background:'#4F46E5',border:'none',color:'#fff',fontWeight:500,cursor:'pointer'}}>{isSavingProfile?'Saving…':'Save'}</button>
                        </div>
                      </div>
                    </div>
                  )}

                  <div style={{padding:'22px 24px'}} ref={scoreBoxes}>
                    {draftData && !hasBeenGraded && (
                      <div style={{background:'#FDF6E7',border:'2px solid #F0C060',borderRadius:'12px',padding:'16px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px',flexWrap:'wrap',marginBottom:'18px'}}>
                        <div>
                          <div style={{color:'#8A5A12',fontWeight:600,fontSize:'0.95rem'}}>You have saved scores for this test</div>
                          {draftData.savedAt && <div style={{color:'#B7791F',fontSize:'0.75rem',marginTop:'2px'}}>Saved {new Date(draftData.savedAt).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</div>}
                        </div>
                        <div style={{display:'flex',gap:'8px',flexShrink:0}}>
                          <button onClick={()=>handleLoadDraft(draftData)} style={{background:'#B7791F',color:'#fff',border:'none',padding:'11px 20px',fontWeight:600,cursor:'pointer',fontSize:'0.9rem',display:'inline-flex',alignItems:'center',gap:'7px'}}><IconRestore/> Restore scores</button>
                          <button onClick={handleDiscardDraft} title="Discard this draft" style={{background:'#fff',color:'#E02424',border:'1.5px solid #DDE2EE',padding:'8px 13px',fontWeight:500,cursor:'pointer',fontSize:'0.82rem'}}>Discard</button>
                        </div>
                      </div>
                    )}
                                        <div style={{display:'flex',gap:'14px',alignItems:'flex-end'}}>
                                            ...
                                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px',gap:'10px',flexWrap:'wrap'}}>
                                              ...
                                              <div style={{display:'flex',gap:'4px',background:'#EEF0F8',padding:'4px',borderRadius:'11px'}}>
                                                {([['graded','Graded'],['absent','Absent'],['na','Not applicable']] as any).map(([val,lbl]:any)=>{
                                                  ...
                                                  return <button key={val} onClick={()=>{ setIsAbsent(val==='absent'); setNotApplicable(val==='na'); }} style={{padding:'7px 14px',borderRadius:'999px',border:'none',cursor:'pointer',fontSize:'0.8rem',fontWeight:active?600:500,background:active?'#fff':'transparent',boxShadow:active?'0 1px 3px rgba(27,31,59,0.12)':'none',color:active?(val==='absent'?'#E02424':val==='na'?'#5A6180':'#4F46E5'):'#5A6180'}}>{lbl}</button>;
                                                })}
                                              </div>
                                              )}
                                            </div>
                                            <select value={assessmentName} onChange={e=>setAssessmentName(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'13px 15px',borderRadius:'12px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#1B1F3B',fontSize:'1rem',fontWeight:600,outline:'none',cursor:'pointer',boxShadow:'0 1px 2px rgba(16,24,40,0.12)'}}>
                                          ...
                                          <div style={{flex:1}}>
                                            ...
                                            {gradedLocked ? (
                                              <div style={{width:'100%',boxSizing:'border-box',padding:'13px 15px',borderRadius:'12px',background:'#EEF0F8',color:'#5A6180',fontSize:'1rem',fontWeight:'600'}}>{lockedData?.maxPoints} pts</div>
                                            ) : (
                                            <select value={maxPoints} onChange={e=>setMaxPoints(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'13px 15px',borderRadius:'12px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#1B1F3B',fontSize:'1rem',fontWeight:600,outline:'none',cursor:'pointer',boxShadow:'0 1px 2px rgba(16,24,40,0.12)'}}>
                                        ...
                                        {gradedLocked ? (
                                        <div style={{background:'#fff',border:'1.5px solid #DDE2EE',borderRadius:'16px',padding:'20px'}}>
                                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px',paddingBottom:'11px',borderBottom:'1.5px solid #DDE2EE'}}>
                                            <span style={{fontSize:'0.9rem',fontWeight:'600'}}>Recorded scores</span>
                                            <button onClick={()=>loadRecordToForm(existingRecord)} style={{background:'#fff',color:'#4F46E5',border:'none',fontSize:'0.8rem',fontWeight:'700',padding:'7px 13px',borderRadius:'9px',cursor:'pointer',boxShadow:'0 1px 2px rgba(16,24,40,0.12)'}}>Edit scores</button>
                                          </div>
                                        ...
                                        {notApplicable && (
                                          <div style={{background:'#fff',borderRadius:'14px',padding:'13px 16px',color:'#475569',fontSize:'0.85rem',lineHeight:'1.5',border:'1.5px solid #DDE2EE'}}>
                                        ...
                                        {!hasBeenGraded && (
                                          <button onClick={handleSaveDraft} disabled={!hasAnyScore} title={!hasAnyScore?'Enter at least one score to save a draft':'Save scores now, add feedback later'} style={{width:'100%',background:hasAnyScore?'#FDF6E7':'#fff',border:`1.5px solid ${hasAnyScore?'#B7791F':'#DDE2EE'}`,color:hasAnyScore?'#B7791F':'#6C7391',padding:'14px',fontWeight:600,fontSize:'0.95rem',cursor:hasAnyScore?'pointer':'not-allowed',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',opacity:hasAnyScore?1:0.6}}>
                                            <IconSave/> Save scores as draft
                                          </button>
                                        )}
                                      </div>
                                    </div>
                                    ...
                                    <div style={{background:'#fff',border:'1px solid rgba(15,23,42,0.06)',borderRadius:'22px',padding:'26px',boxShadow:'0 1px 2px rgba(16,24,40,0.04), 0 14px 30px -12px rgba(16,24,40,0.16)',flexShrink:0}}>
                                      <div style={{marginBottom:'18px'}}>
                                        ...
                                        <p style={{margin:0,fontSize:'0.82rem',color:'#68738A'}}>Notes stay private. Feedback is emailed to the student.</p>
                                      </div>
                                      ...
                                      <div style={{marginBottom:'18px'}}>
                                        ...
                                        <textarea value={teacherNotes} onChange={e=>setTeacherNotes(e.target.value)} placeholder="e.g., struggled with present continuous; speaking fluency improving but hesitant..." rows={2} style={{width:'100%',boxSizing:'border-box',padding:'13px 14px',borderRadius:'13px',border:'1.5px solid #DDE2EE',background:'#FAFBFD',color:'#334155',fontSize:'0.98rem',outline:'none',resize:'vertical',lineHeight:'1.55'}}/>
                                      </div>
                                      ...
                                      <div style={{marginBottom:'20px'}}>
                                          ...
                                          <div style={{background:'#FAFBFD',border:'1px solid #E7EAF1',borderRadius:'14px',padding:'16px',marginBottom:'14px'}}>
                                            {builderEditing ? (
                                              <>
                                                ...
                                                <p style={{margin:'0 0 12px',fontSize:'0.72rem',color:'#68738A'}}>Lowercase phrases work best — they slot into "for reading, …". Saved on this device.</p>
                                                    ...
                                                    <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                                                      {(feedbackTips[sk.key]||[]).map((tip,i)=>(
                                                        <div key={i} style={{display:'flex',gap:'6px',alignItems:'center'}}>
                                                          <input value={tip} onChange={e=>editTip(sk.key,i,e.target.value)} style={{flex:1,boxSizing:'border-box',padding:'8px 10px',borderRadius:'9px',border:'1.5px solid #DDE2EE',background:'#fff',fontSize:'0.8rem',color:'#334155',outline:'none'}}/>
                                                ...
                                                <div style={{borderTop:'1px solid #DDE2EE',margin:'4px 0 12px'}}/>
                                                {PHRASE_CATEGORIES.map(cat=>(
                                                  <div key={cat.key} style={{marginBottom:'14px'}}>
                                                    ...
                                                    <div style={{fontSize:'0.68rem',color:'#68738A',marginBottom:'6px'}}>{cat.hint}</div>
                                                    <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                                                      {(feedbackTips[cat.key]||[]).map((tip,i)=>(
                                                        <div key={i} style={{display:'flex',gap:'6px',alignItems:'center'}}>
                                                          <input value={tip} onChange={e=>editTip(cat.key,i,e.target.value)} style={{flex:1,boxSizing:'border-box',padding:'8px 10px',borderRadius:'9px',border:'1.5px solid #DDE2EE',background:'#fff',fontSize:'0.8rem',color:'#334155',outline:'none'}}/>
                                                ...
                                                <button onClick={resetTips} style={{background:'transparent',border:'1.5px solid #DDE2EE',color:'#64748B',padding:'8px 14px',borderRadius:'9px',fontSize:'0.78rem',fontWeight:'500',cursor:'pointer'}}>Reset to starter phrases</button>
                                              </>
                                            ) : (
                                              <>
                                                ...
                                                <textarea value={builderNote} onChange={e=>{setBuilderNote(e.target.value);setBuilderConfirmReplace(false);}} placeholder="Something specific to this student — a class moment, particular praise, anything…" rows={2} style={{width:'100%',boxSizing:'border-box',padding:'9px 11px',borderRadius:'10px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#334155',fontSize:'0.82rem',outline:'none',resize:'vertical',lineHeight:'1.5',marginBottom:'12px'}}/>
                                                ...
                                                {INSIGHT_SKILLS.map(sk=>{
                                                  ...
                                                  return (
                                                    <div key={sk.key} style={{borderTop:'1px solid #DDE2EE'}}>
                                                      <div onClick={()=>toggleSet(setBuilderExpanded,sk.key)} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 0',cursor:'pointer'}}>
                                                        ...
                                                        <span style={{color:'#68738A',fontSize:'0.8rem'}}>{expanded?'▾':'▸'}</span>
                                                      </div>
                                                      {expanded && (
                                                ...
                                                <label style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'0.85rem',color:'#334155',cursor:'pointer',margin:'12px 0 14px',borderTop:'1px solid #DDE2EE',paddingTop:'12px'}}>
                                          ...
                                          <div style={{border:'1.5px solid #DDE2EE',borderRadius:'14px',padding:'16px',marginBottom:'14px',background:'#fff'}}>
                                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'10px',flexWrap:'wrap',marginBottom:'10px'}}>
                                              ...
                                              <button onClick={()=>setShowLessons(true)} disabled={!currentLevel}
                                                ...
                                                style={{background:'#fff',border:'1.5px solid #DDE2EE',color:'#5A6180',padding:'6px 14px',fontSize:'0.8rem',cursor:'pointer'}}>
                                                Lessons for this test
                                              </button>
                                            </div>
                                            ...
                                            <button onClick={()=>{ setFeedback(feedbackPlan.text); showToast('Feedback written','success'); }}
                                              style={{width:'100%',background:'#4F46E5',border:'none',color:'#fff',padding:'11px',fontWeight:600,fontSize:'0.9rem',cursor:'pointer'}}>
                                              Write the feedback
                                            </button>
                                          </div>
                                        )}
                                        ...
                                        {gradedLocked ? (
                                          <div style={{width:'100%',boxSizing:'border-box',padding:'13px 15px',borderRadius:'13px',border:'1.5px solid #DDE2EE',background:'#F8FAFC',color:'#334155',fontSize:'0.98rem',lineHeight:'1.6',whiteSpace:'pre-wrap',minHeight:'80px'}}>{existingRecord?.feedback || <span style={{color:'#68738A'}}>No written feedback was saved for this test.</span>}</div>
                                        ) : (
                                        <textarea data-feedback-box value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Generate a draft with AI, then edit — or write your own..." rows={4} style={{width:'100%',boxSizing:'border-box',padding:'13px 14px',borderRadius:'13px',border:'1.5px solid #DDE2EE',background:'#FAFBFD',color:'#334155',fontSize:'0.98rem',outline:'none',resize:'vertical',lineHeight:'1.6'}}/>
                                        )}
                                        ...
                                        {projectedInsights && projectedInsights.count > 1 && (!hasBeenGraded || editingRecordId) && (
                                          <div style={{display:'flex',alignItems:'center',gap:'8px',marginTop:'10px',flexWrap:'wrap'}}>
                                            ...
                                            <select value={emailFocus} onChange={e=>setEmailFocus(e.target.value)} style={{padding:'6px 10px',borderRadius:'9px',border:'1.5px solid #DDE2EE',background:'#fff',color:'#334155',fontSize:'0.8rem',outline:'none',cursor:'pointer'}}>
                  ...
                  <div style={{padding:'16px 24px',borderTop:'1px solid #DDE2EE',background:'#F7F8FD'}}>
                                      ...
                                      {editingRecordId ? (
                                        ...
                                        <>
                                          ...
                                          <p style={{margin:'10px 2px 0',fontSize:'0.78rem',color:'#6C7391',textAlign:'center'}}>Save grade records it now without emailing — you can send the email later from Previous records.</p>
                                        </>
                                      )}
                  </div>
                </div>

                                {/* ── Term summary (collapsible, like the two panels below) ── */}
                                <div style={{flexShrink:0}}>
                                  <button onClick={()=>setShowTermSummary(v=>!v)} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#fff',border:'1px solid rgba(15,23,42,0.06)',borderRadius:'16px',padding:'14px 18px',cursor:'pointer',boxShadow:'0 1px 2px rgba(16,24,40,0.04)',gap:'12px'}}>
                                    <span style={{display:'flex',alignItems:'center',gap:'10px',flexWrap:'wrap'}}>
                                      <span style={{color:'#0F172A',fontSize:'1.02rem',fontWeight:'600'}}>Term summary</span>
                                      {termSummary?.standing!=null && (
                                        <span style={{background:'#EEF2FF',color:'#4F46E5',fontSize:'0.72rem',fontWeight:'700',padding:'2px 9px',borderRadius:'999px'}}>
                                          {Math.round(termSummary.standing)}%
                                        </span>
                                      )}
                                    </span>
                                    <span style={{color:'#68738A',fontSize:'0.85rem',fontWeight:'500',whiteSpace:'nowrap'}}>{showTermSummary?'Hide':'Show'}</span>
                                  </button>
                                  {showTermSummary && <div style={{marginTop:'14px'}}><TermSummaryCard summary={termSummary} /></div>}
                                </div>
              ...
              <div style={{background:'#fff',borderRadius:'18px',padding:'56px 40px',textAlign:'center',boxShadow:'0 2px 8px rgba(27,31,59,0.05)'}}>
                <h3 style={{margin:'0 0 8px',color:'#1B1F3B',fontSize:'1.3rem'}}>Select a student</h3>
                <p style={{margin:0,color:'#6C7391',fontSize:'0.95rem'}}>Choose someone from the list to record a grade.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <TestLessons
        ...
        getSupabase={async()=>getSupabaseClient((await getToken({template:'supabase'}))||'')}
        onSaved={(m)=>{ setLessonMap(m); setTickedLessons(null); showToast('Lessons saved','success'); }}
      />

...
export const LivePlayer: React.FC = () => {
  ...
  useEffect(() => {
    if (session?.status === 'active' && !isLoadingQuestions && questions.length > 0 && !isFinished && !feedbackState?.show) {
      ...
      setTimeLeft(20); 
      const timerInterval = setInterval(() => { setTimeLeft((prev) => prev <= 1 ? 0 : prev - 1); }, 1000);
  ...
  const handleAnswer = async (selectedKey: string | null) => {
    ...
    let newScore = score;
    if (isCorrect) {
      const timeRatio = Math.min(timeTakenMs / 20000, 1); 
          ...
          {isLoadingQuestions ? (
            ...
            <div style={{ background: '#ffffff', padding: '32px', borderRadius: '32px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', border: `1px solid ${theme.border}` }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <span style={{ color: timeLeft > 5 ? '#0F172A' : '#EF4444', fontWeight: '800', fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
